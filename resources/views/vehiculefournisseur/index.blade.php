    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="flex justify-between items-center">
                <h2 class="text-2xl font-bold">Liste des véhicules fournisseur</h2>
                <a class="btn btn-light" href="{{ route('dashboard.vehiculefournisseur.create') }}">Ajouter un
                    véhicule</a>
            </div>
                        ...
                        @foreach ($vehicule as $vehicul)
                            <tr class="hover:bg-gray-100">
                                ...
                                <td class="px-4 py-2 text-center">
                                    {{ \Carbon\Carbon::parse($vehicul->last_maintenance)->format('d/m/Y') }}</td>
                                ...
                                <td class="text-center">
                                    <a class="btn btn-light btn-block" role="button"
                                        href="{{ route('dashboard.vehiculefournisseur.edit', ['id' => $vehicul->id]) }}">Modifier</a>
                                    <form
                                        action="{{ route('dashboard.vehiculefournisseur.delete', ['id' => $vehicul->id]) }}"
                                        method="post">
                                        @csrf
                                        @method('DELETE')
