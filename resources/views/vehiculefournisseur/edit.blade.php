        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
          <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
              ...
              @endif
              <form action="{{ route('dashboard.vehiculefournisseur.update', ['id' => $vehicule->id]) }}" method="post">
                @csrf
                @method('PUT')
                <div class="overflow-x-auto">
                  <table class="table-auto w-full">
                    ...
                    <tbody>
                      <tr>
                        <td class="border px-4 py-2"><input type="text" name="model" placeholder="Modèle" value="{{ $vehicule->model }}" class="w-full"></td>
                        <td class="border px-4 py-2"><input type="text" name="marque" placeholder="Marque" value="{{ $vehicule->marque }}" class="w-full"></td>
                        <td class="border px-4 py-2"><input type="date" name="last_maintenance" value="{{ \Carbon\Carbon::parse($vehicule->last_maintenance)->format('Y-m-d') }}" class="w-full"></td>
                        <td class="border px-4 py-2"><input type="text" name="nb_kilometrage" placeholder="kilomètres" value="{{ $vehicule->nb_kilometrage }}" class="w-full"></td>
                        <td class="border px-4 py-2"><input type="text" name="nb_serie" placeholder="N° de série" value="{{ $vehicule->nb_serie }}" class="w-full"></td>
